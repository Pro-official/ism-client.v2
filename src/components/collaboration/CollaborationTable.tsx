  ...
  return (
    <div className="fixed inset-0 bg-gray-900/70 z-50 flex items-center justify-center">
      ...
      >
        ...
        <p className="text-white/90 mb-6">{actionMessage}</p>
...
export default function CollaborationTable() {
  ...
  const fetchCollaborators = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://ism-serverv2.onrender.com/api/ideas/${ideaId}/collaborators`
      );
      if (!response.ok) {
  ...
  const handleStatusChange = async (
    ...
    try {
      const response = await fetch(
        `https://ism-serverv2.onrender.com/api/ideas/${ideaId}/assign-role`,
        {
          method: "PATCH",
          headers: {
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <p className="text-white/90 text-xs border-2 rounded-full w-8 h-8 p-2 flex items-center justify-center bg-gradient-to-r from-green-500 to-green-600">
                        {collaborator?.name
                          .split(" ")
                          .map((word) => word[0])
                          .join("")}
                    ...
                    {user?.role === "Regional" ? (
                      <div className="flex gap-2 items-center">
                        <select
                          ...
                          className="bg-gray-600 rounded-md border border-white/50 py-1 px-2 text-white/90"
                        >
