  ...
  return (
    <div className="fixed inset-0 bg-gray-900/70 z-50 flex items-center justify-center">
      <motion.div
        ...
        className="bg-black/10 backdrop-blur-lg border border-white/50 p-8 rounded-2xl shadow-lg max-w-md"
      >
        ...
        <div className="flex justify-end gap-4">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg bg-gray-700 text-white/90 hover:bg-gray-600 transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="px-4 py-2 rounded-lg bg-gradient-to-r from-green-500 to-green-600 text-white/90 font-medium hover:from-green-600 hover:to-pink-600 transition-colors"
          >
            Confirm
          </button>
        </div>
...
export default function StatusBadge({
  ...
  const statusConfig: {
    ...
    "Idea Pending": {
      text: "Idea Pending",
      colors: "bg-gray-400 text-white/90",
    },
    "Idea in Progress": {
      text: "Idea in Progress",
      colors: "bg-yellow-400 text-white/90",
    },
    "Idea Evaluated": {
      text: "Idea Evaluated",
      colors: "bg-green-400 text-white/90",
    },
  };
      ...
      if (user) {
        try {
          const response = await fetch(
            `https://ism-serverv2.onrender.com/api/ideas/${ideaId}/status`,
            {
              method: "PATCH",
              headers: {
  ...
  const handleConfirmCollaboration = async () => {
    if (user) {
      try {
        const response = await fetch(
          `https://ism-serverv2.onrender.com/api/ideas/${ideaId}/req-collaboration`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    <div className="relative">
      ...
      {isDropdownOpen && type !== "collaborate" && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 10 }}
          className="absolute top-full mt-1 right-0 w-48 bg-black/10 backdrop-blur-lg border border-white/50 rounded-2xl shadow-lg z-10"
        >
          {statusOptions.map((statusOption) => (
            <motion.button
              key={statusOption}
              onClick={() => handleStatusChange(statusOption)}
              className="w-full text-left px-4 py-2 text-white/90 hover:bg-black/10/10 block rounded-t-lg last:rounded-b-lg"
            >
              {statusOption}
            </motion.button>
          ))}
