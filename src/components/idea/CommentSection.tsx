  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (newComment.trim() && user) {
        try {
          const response = await fetch(
            `https://ism-serverv2.onrender.com/api/ideas/${ideaId}/comment`,
            {
              method: "POST",
              headers: {
    ...
    <div className="space-y-8">
      <form onSubmit={handleSubmit} className="space-y-4">
        <textarea
          ...
          className="w-full p-4 rounded-lg bg-black/10 border border-white/50 text-white/90 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 min-h-[100px]"
        />
      ...
      <div className="space-y-6">
        <AnimatePresence>
          {comments
            .map((comment) => (
              <motion.div
                ...
                exit={{ opacity: 0, y: -20 }}
                className="p-6 rounded-lg bg-black/10 border border-white/50"
              >
          ...
          >
            ...
            <p className="text-white/90">{mockComments.comment}</p>
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
